...
// 이미지를 최대 1500px, JPEG 0.82로 압축 (localStorage 용량 절약)
...
async function processPair(pages, index, total, batchTs) {
  ...
  // 처리 중 상태: imageData 저장 안 함 (용량 절약)
  const app = {
    id,
  ...
  try {
    ...
    // IndexedDB에 압축 이미지 저장 (localStorage 용량 한도 우회)
    await Promise.all(
      compressedURLs.map((url, pi) => savePageImage(id, pi, url)),
    );
    ...
    updateApplication(id, {
      ...
      pages: pages.map((p, pi) => ({
        imageData: null, // localStorage에는 이미지 저장 안 함
        filename: p.file.name,
        fields: fieldSets[pi],
    ...
    updateApplication(id, { status: 'review', applicantName: '오류' });
...
function getSonanongResult(app) {
  const p1 = app.pages?.[0]?.fields || [];
  const p2 = app.pages?.[1]?.fields || [];
  const fv = (key) => getFieldValue([...p1, ...p2], key);
  const condKeys = [
    'appliedArea',
    'ownedArea',
    'farmingYears',
  ...
  const results = condKeys.map(
    (key) => evaluateCondition(key, fv(key))?.status,
  );
...
function buildRow(app, isTrash) {
  ...
  if (app.status === 'completed') {
    ...
    const maskedId = maskResidentId(getFieldValue(allFields, 'residentId'));
    meta =
  ...
  return `<div class="app-row">
    <div class="app-row-name">${name} ${badge}${sonanongBadge}</div>
...
function doHardDelete(id) {
  if (!confirm('영구 삭제하면 복원할 수 없습니다. 삭제하시겠습니까?')) return;
  hardDeleteFromTrash(id);
  deleteAppImages(id); // IndexedDB 이미지도 정리
  renderList();
}
...
function confirmEmptyTrash() {
  ...
  if (
    !confirm(`휴지통의 신청서 ${trash.length}건을 모두 영구 삭제하시겠습니까?`)
  )
    return;
  trash.forEach((a) => deleteAppImages(a.id)); // IndexedDB 이미지 일괄 정리
  emptyTrash();
  renderList();
}
...
function getExportRows() {
  return loadApplications()
    ...
    .map((app) => {
      ...
      const fv = (key) => getFieldValue([...p1, ...p2], key);
      const ev = (key) => {
        const r = evaluateCondition(key, fv(key));
