...
// 접수유형 4분류 배지 (현장종이/현장모바일·직원동반/재신청·변경있음/재신청·변경없음)
const RECEPTION_BADGE_STYLE = {
  staff:             'background:var(--gray-100);color:var(--gray-500);',
  citizen:           'background:#E6F7EC;color:#1F9D57;',
  assisted:          'background:#E8F1FF;color:#3169C6;',
  reapply_changed:   'background:var(--warning-light);color:var(--warning);',
  reapply_no_change: 'background:#F0EAFB;color:#7C5CBF;',
};
function sourceBadgeHTML(app) {
  const t = getReceptionType(app);
  return `<span class="status-badge" style="${RECEPTION_BADGE_STYLE[t.key] || ''}">${t.icon} ${t.label}</span>`;
}
...
function buildRow(app, isTrash) {
  ...
  // 완료 탭: 소농 판정 뱃지 + 면적직불금 예상액 + 주민번호 마스킹 메타
  let sonanongBadge = '';
  ...
  if (app.status === 'completed') {
    ...
    sonanongBadge =
      result === 'pass'
        ? '<span class="cond-badge pass" style="font-size:11px;margin-left:6px;">소농 충족</span>'
        : result === 'fail'
          ? '<span class="cond-badge fail" style="font-size:11px;margin-left:6px;">소농 미충족 · 면적만 가능</span>'
          : '<span class="cond-badge unknown" style="font-size:11px;margin-left:6px;">소농 판정 불가</span>';
    ...
    meta =
      ...
      (maskedId ? ` · ${maskedId}` : '') +
      (areaEst.amount != null ? ` · 면적직불 예상 ${areaEst.amount.toLocaleString()}원` : '') +
      (result === 'pass' ? ` · 소농직불 ${SONANONG_PAYMENT.toLocaleString()}원` : '');
  }
...
function getExportRows() {
  return cachedApps
    ...
    .map((app) => {
      ...
      const sonanong = getSonanongResult(app);
      const areaEst = estimateAreaPaymentForApp(app);
      return {
        신청서ID: app.id,
        ...
        '소농직불금 판정': sonanong === 'pass' ? '충족' : sonanong === 'fail' ? '미충족' : '판정불가',
        '소농직불금(원)': sonanong === 'pass' ? SONANONG_PAYMENT : '',
