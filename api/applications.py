...
@bp.get('/applications/lookup')
def lookup_previous():
    """재신청자용: 주민번호로 가장 최근 완료 신청서 조회"""
    resident_id = request.args.get('residentId', '')
    app = repository.find_latest_completed_by_resident_id(resident_id)
    if app is None:
        return jsonify({'error': '이전 신청 기록을 찾을 수 없습니다'}), 404
    return jsonify(app)
